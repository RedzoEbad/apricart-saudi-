...
public class ProductController {
    ...
    public ResponseEntity<GenericResponse<List<?>>> findProductsByCategoryId(
    ...
    @GetMapping("/zoho/{id}")
    public ResponseEntity<GenericResponse<List<ProductResponseDTO>>> findProductsByZohoId(
            @RequestHeader(HttpHeaders.AUTHORIZATION) String authorizationHeader,
            @RequestHeader("Language") LanguageType lang,
            @PathVariable Long id,
            @RequestParam(defaultValue = "0", required = false) int pageNo,
    ...
    public ResponseEntity<GenericResponse<List<?>>> findProductsBySubCategoryId(
            @RequestHeader(HttpHeaders.AUTHORIZATION) String authorizationHeader,
            @RequestHeader("Language") LanguageType lang,
            @PathVariable Long id,
            @RequestParam(defaultValue = "0", required = false) int pageNo,
    ...
    public ResponseEntity<GenericResponse<List<ProductResponseDTO>>> findProductsByBrandId(
            @RequestHeader(HttpHeaders.AUTHORIZATION) String authorizationHeader,
            @RequestHeader("Language") LanguageType lang,
            @PathVariable Long id,
            @RequestParam(defaultValue = "0", required = false) int pageNo,
    ...
    @GetMapping("/trending")
    public ResponseEntity<GenericResponse<List<ProductResponseDTO>>> findTrendingProducts(
            @RequestHeader(HttpHeaders.AUTHORIZATION) String authorizationHeader,
            @RequestHeader("Language") LanguageType lang,
            @RequestParam(defaultValue = "0", required = false) int pageNo,
            @RequestParam(defaultValue = "10", required = false) int pageSize) {
        List<ProductResponseDTO> products = Product
                .toDTOList(productService.getTrendingProducts(lang, pageNo, pageSize));
        return !products.isEmpty() ? Response.success(products) : Response.notFound();
    }

    @ApiOperation(value = "Get Discounted Products", authorizations = { @Authorization(value = "jwtToken") })
    @GetMapping("/discounted")
    public ResponseEntity<GenericResponse<List<ProductResponseDTO>>> findDiscountedProducts(
            @RequestHeader(HttpHeaders.AUTHORIZATION) String authorizationHeader,
            @RequestHeader("Language") LanguageType lang,
            @RequestParam(defaultValue = "0", required = false) int pageNo,
            @RequestParam(defaultValue = "10", required = false) int pageSize) {
        List<ProductResponseDTO> products = Product
                .toDTOList(productService.getDiscountedProducts(lang, pageNo, pageSize));
        return !products.isEmpty() ? Response.success(products) : Response.notFound();
    }

    @ApiOperation(value = "Get New Arrival Products", authorizations = { @Authorization(value = "jwtToken") })
    @GetMapping("/newarrivals")
    public ResponseEntity<GenericResponse<List<ProductResponseDTO>>> findNewArrivalsProducts(
            @RequestHeader(HttpHeaders.AUTHORIZATION) String authorizationHeader,
            @RequestHeader("Language") LanguageType lang,
            @RequestParam(defaultValue = "0", required = false) int pageNo,
            @RequestParam(defaultValue = "10", required = false) int pageSize) {
        List<ProductResponseDTO> products = Product
                .toDTOList(productService.getNewArrivalsProducts(lang, pageNo, pageSize));
        return !products.isEmpty() ? Response.success(products) : Response.notFound();
    }

    @ApiOperation(value = "Get Featured Products", authorizations = { @Authorization(value = "jwtToken") })
    @GetMapping("/featured")
    public ResponseEntity<GenericResponse<List<ProductResponseDTO>>> findFeaturedProducts(
            @RequestHeader(HttpHeaders.AUTHORIZATION) String authorizationHeader,
            @RequestHeader("Language") LanguageType lang,
            @RequestParam(defaultValue = "0", required = false) int pageNo,
            @RequestParam(defaultValue = "10", required = false) int pageSize) {
        List<ProductResponseDTO> products = Product
                .toDTOList(productService.getFeaturedProducts(lang, pageNo, pageSize));
        return !products.isEmpty() ? Response.success(products) : Response.notFound();
    }
    ...
    public ResponseEntity<GenericResponse<ProductResponseDTO>> updateProductPositionById(
        ...
        return updateProductPosition != null ? Response.success(Product.toDTO(updateProductPosition))
                : Response.notFound();
    }
    ...
    public ResponseEntity<GenericResponse<String>> deleteCategory(
        ...
        return lang.equals(LanguageType.ARB) ? Response.success(PRODUCT_REMOVED_SUCCESSFULLY_ARABIC)
    ...
    private ResponseEntity<GenericResponse<List<?>>> getDiscountedProductsResponse(LanguageType lang, Integer pageNo,
            Integer pageSize) {
        List<ProductResponseDTO> products = Product
                .toDTOList(productService.getDiscountedProducts(lang, pageNo, pageSize));
    ...
    private ResponseEntity<GenericResponse<List<?>>> getProductsResponseCategory(Long categoryId, int pageNo,
            int pageSize, LanguageType languageType) {
        List<ProductResponseDTO> products = Product
                .toDTOList(productService.findByCategoryId(categoryId, pageNo, pageSize, languageType));
        return !products.isEmpty() ? Response.success(products) : Response.notFound();
    }

    private ResponseEntity<GenericResponse<List<?>>> getProductsResponseSubCategory(Long subCategoryId, int pageNo,
            int pageSize, LanguageType languageType) {
        List<ProductResponseDTO> products = Product
                .toDTOList(productService.findBySubCategoryId(subCategoryId, pageNo, pageSize, languageType));
        return !products.isEmpty() ? Response.success(products) : Response.notFound();
    }
}
